...
export default function ProjectsDashboard() {
  ...
  const loadNodes = async () => {
    try {
      ...
      if (projData) {
        // Fetch memories to get accurate file counts
  ...
  const handleSyncTrigger = async () => {
    ...
    try {
      ...
      if (result.upgrade) {
        ...
        await loadNodes() // Refresh file counts
  ...
  return (
    <div className="flex-1 overflow-y-auto max-h-screen bg-[#050505] custom-scrollbar relative">
          ...
          {projects?.length === 0 ? (
            ...
            projects?.map((project) => {
              ...
              return (
                <div 
                  key={project.id}
                  className="group relative bg-[#0a0a0a] border border-gray-800 hover:border-blue-500/50 rounded-2xl p-6 transition-all duration-300 hover:shadow-[0_0_30px_rgba(37,99,235,0.05)] flex flex-col cursor-pointer h-full"
                  onClick={() => router.push(`/dashboard/projects/${project.id}/doc`)}
                >
                  {/* Top Row: Icon, Name & Status Glow */}
      ...
      {/* Sync Pipeline Modal (With Premium Neon Provider Icons) */}
      {isSyncModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
