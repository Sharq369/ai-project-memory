        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <div key={project.id} className="bg-[#111319] border border-gray-800/40 rounded-[2.5rem] p-8 relative">
              <div className="w-12 h-12 rounded-full bg-blue-500/10 flex items-center justify-center mb-8 border border-blue-500/20"><Globe size={20} className="text-blue-500" /></div>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 pointer-events-auto">
          <div className="bg-[#0f1116] border border-gray-800 rounded-[3rem] p-10 w-full max-w-sm relative shadow-2xl">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-2xl font-black italic uppercase text-white">Source</h3>
              <button onClick={() => setSelectedNode(null)} className="text-gray-500"><X size={24} /></button>
            </div>
            <div className="space-y-4">
              <button onClick={() => alert("GitHub")} className="w-full flex items-center gap-4 bg-[#16181e] p-5 rounded-2xl border border-gray-800 text-white text-[10px] font-black uppercase tracking-widest">
                <Github size={20}/> GITHUB PROTOCOL
              </button>
              <button onClick={() => alert("GitLab")} className="w-full flex items-center gap-4 bg-[#16181e] p-5 rounded-2xl border border-gray-800 text-white text-[10px] font-black uppercase tracking-widest">
