...
import { 
  ...
  X, Pencil, Github, Gitlab, Cloud, Terminal, Box, Check, Copy, Zap, Eye
} from 'lucide-react'
...
export default function ProjectDocPage() {
  ...
  // --- NEW: HANDLE FILE COPY ---
  const handleCopyFileCode = async (content: string) => {
    ...
    <div className="min-h-screen bg-[#0a0b0e] text-white flex overflow-hidden relative font-sans">
      ...
      <div className={`flex-1 p-6 lg:p-12 transition-all duration-500 overflow-y-auto ${chatOpen ? 'mr-[400px]' : ''}`}>
        ...
        <div className="max-w-5xl mx-auto">
          <header className="bg-[#111319] border border-gray-800/40 p-6 lg:p-10 rounded-[2.5rem] flex flex-col lg:flex-row justify-between items-start mb-10 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex-1 w-full">
              <div className="flex items-center gap-4 mb-8">
                {isEditing ? (
                  <div className="flex items-center gap-4 w-full">
                    <input autoFocus value={editName} onChange={(e) => setEditName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleRename()} className="bg-black/50 border border-blue-600 rounded-xl px-4 py-2 text-2xl lg:text-5xl font-black italic uppercase tracking-tighter text-white outline-none w-full max-w-md"/>
                    <button onClick={handleRename} className="p-3 bg-blue-600 rounded-lg"><Check size={18} /></button>
                  </div>
                ) : (
                  <>
              ...
              <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
                ...
                <div className="hidden lg:block w-px h-16 bg-gray-800 mt-4"></div>
            ...
            <button onClick={() => setChatOpen(!chatOpen)} className="mt-8 lg:mt-0 bg-blue-600 p-6 lg:p-7 rounded-[2.2rem] hover:scale-105 transition-all shadow-xl shadow-blue-900/40 lg:ml-8 relative">
          ...
          <div className="flex justify-between items-center mb-8">
            ...
            <button onClick={copyNeuralContext} disabled={memories.length === 0} className={`flex items-center gap-2 px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${copied ? 'bg-green-600 text-white' : 'bg-[#111319] border border-blue-600/30 text-blue-400'}`}>
              {copied ? <Check size={14} /> : <Copy size={14} />} {copied ? 'CONTEXT SYNCED' : 'COPY NODE CONTEXT'}
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 pb-32">
            {memories.map((mem) => (
              <div 
                ...
                className="bg-[#111319] border border-gray-800/40 rounded-[2rem] overflow-hidden group hover:border-blue-600/50 transition-all cursor-pointer shadow-lg active:scale-[0.98]"
              >
                <div className="flex justify-between items-center p-6 border-b border-gray-800/40 bg-white/[0.01]">
                  <div className="flex items-center gap-4">
                    <Terminal size={18} className="text-blue-500 group-hover:animate-pulse"/>
                    <div>
                      <h3 className="text-[10px] font-black uppercase tracking-[0.2em] group-hover:text-blue-400 transition-colors">{mem.file_name}</h3>
                      <p className="text-[7px] text-gray-600 font-bold uppercase mt-1">Status: Verified</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <Eye size={12} className="text-gray-600 group-hover:text-white" />
                    <span className="text-[8px] font-black uppercase px-3 py-1.5 rounded-lg bg-green-500/10 border border-green-500/20 text-green-500 flex items-center gap-2">
                       <Box size={10}/> GROUNDED
                    </span>
                  </div>
                </div>
                <div className="p-6">
                  <pre className="p-6 bg-black/60 rounded-[1.2rem] text-[10px] font-mono text-gray-500 overflow-hidden line-clamp-3 border border-gray-800/20">
            ...
            {memories.length === 0 && !loading && (
              <div className="text-center p-20 border border-dashed border-gray-800 rounded-[2.5rem] bg-[#111319]/30">
                ...
                <button onClick={() => setIsSyncModalOpen(true)} className="px-8 py-4 bg-blue-600 hover:bg-white hover:text-black rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 mx-auto">
      ...
      {/* --- CODE PREVIEW MODAL --- */}
      {selectedFile && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[200] flex items-center justify-center p-4 lg:p-12 animate-in fade-in duration-300">
          <div className="bg-[#0a0b0e] border border-gray-800/50 w-full max-w-6xl h-[90vh] rounded-[2.5rem] flex flex-col shadow-[0_0_100px_rgba(37,99,235,0.1)] overflow-hidden">
            
            <div className="flex items-center justify-between px-8 py-6 border-b border-gray-800/60 bg-[#111319]">
              <div className="flex items-center gap-4">
                <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></div>
                <h2 className="text-[12px] font-black italic uppercase tracking-[0.2em] text-white">
              ...
              <div className="flex items-center gap-6">
                <button 
                  onClick={() => handleCopyFileCode(selectedFile.content)}
                  className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-gray-400 hover:text-blue-400 transition-colors"
                >
                  {fileCopied ? <Check size={14} className="text-green-500" /> : <Copy size={14} />}
                  {fileCopied ? 'COPIED' : 'COPY NODE'}
                </button>
                <button 
                  onClick={() => setSelectedFile(null)} 
                  className="bg-white/5 hover:bg-white/10 p-2.5 rounded-xl text-gray-500 hover:text-white transition-all"
                >
                  <X size={20}/>
                </button>
              </div>
            ...
            <div className="flex-1 overflow-auto bg-[#0a0b0e] p-8 custom-scrollbar">
              <pre className="text-[13px] leading-relaxed font-mono text-blue-100/80 whitespace-pre">
      ...
      {isSyncModalOpen && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-[#111319] border border-gray-800 w-full max-w-sm rounded-[2.5rem] p-10 relative shadow-2xl">
            <button onClick={() => setIsSyncModalOpen(false)} className="absolute top-8 right-8 text-gray-600 hover:text-white transition-colors"><X size={18}/></button>
            ...
            <p className="text-[9px] text-gray-500 font-black uppercase tracking-widest mb-8">Fetch repository files into this Neural Node.</p>
            <input autoFocus value={repoName} onChange={(e) => setRepoName(e.target.value)} placeholder="e.g., owner/repo" className="w-full bg-black/40 border border-gray-800 rounded-xl px-4 py-4 text-[10px] font-black uppercase text-white outline-none focus:border-blue-600 mb-6 transition-all"/>
            <button onClick={handleSyncTrigger} disabled={isSyncing || !repoName.trim()} className={`w-full py-4 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${isSyncing ? 'bg-blue-600/50' : 'bg-blue-600 hover:bg-white hover:text-black'}`}>
              {isSyncing ? <><Loader2 size={14} className="animate-spin"/> SYNCING...</> : <><Zap size={14}/> INITIATE SYNC</>}
            </button>
          </div>
        </div>
      )}
      ...
      <div className={`fixed right-0 top-0 h-full w-[400px] bg-[#0d0f14] border-l border-gray-800/50 shadow-2xl transition-transform duration-500 z-[150] ${chatOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-full flex flex-col p-10">
          ...
          <div className="relative">
            <input value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="QUERY ARCHIVE..." className="w-full bg-[#111319] border border-gray-800 rounded-2xl py-6 pl-8 pr-16 text-[10px] font-black uppercase text-white outline-none focus:border-blue-600 transition-all shadow-lg"/>
